...
module Of_xml : sig
  ...
  val parse_failure : ?path_rev:Xml.Tag.t list -> ('a, unit, string, 'b) format4 -> 'a
  ...
  val check_no_extra_attributes : ?path_rev:Xml.Tag.t list -> Xml.Attribute.t list -> unit
  ...
  val parse : ?path_rev:Xml.Tag.t list -> 'a t -> Xml.element -> 'a
  ...
  val override_parse
    ...
    -> f:
         (namespace:Namespace.t
          -> tag:string
          -> (?path_rev:Xml.Tag.t list -> Xml.element -> 'a)
          -> ?path_rev:Xml.Tag.t list
          -> Xml.element
          -> 'b)
    -> 'b t
